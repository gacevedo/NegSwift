...
| **S0–S15** Native Swift engine | **S13 reopened** | S13a–j done (slider reprints + one decode + Accelerate + splash/thumbs + progressive first paint + GPU present + X-Trans PPG + queue/prefetch). Next: **S13k** disk cache; **S13m** export perf. S14 RAW look is in. S15 later. See §14 |

**Resume here:** Native engine **S13k** (longer-lived caches) and **S13m** (export performance). S13a–j shipped (slider reprints, one linear decode per file, vImage/vDSP convert/resize, embedded-preview splash + cheap strip thumbs, progressive draft / refine first paint, GPU present without float readback, X-Trans preview PPG + one libraw handle per file, parallel TIFF/JPEG decode with selected-frame priority and neighbor linear prefetch). S9 export look is in (`original` + `target_px` downsample); export still runs full-res pipeline unless S13m lands. S14 camera RAW is in (oracle for S13i). S15 iOS stays later while S13 is open. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md).
...
- [x] Submodule SHA pinned to NegPy **0.58.0** (validated by engine tests)
...
**Coordinate mapping:** Preview clicks are in **display-normalized** space (0–1 in the bitmap Swift shows). NegPy stores strokes in **source-normalized** space. With rotation/fine rotation they differ. Do **not** reimplement `CoordinateMapping` in the Swift UI. The native engine (**S10a**) must bake heals in source space on the decoded linear buffer, before geometry — same contract as NegPy.
...
**Parity rule:** The Python backend does not reimplement math; if that preview differs from NegPy desktop, treat as **engine wiring bug**, not “Swift UI bug”. `Packages/NegSwiftEngine` is the exception: compare it to Python at the vertical’s **pinned config**, not to app defaults until S5+S8.
...
5. **Native engine S13:** First-load performance — start at **S13k** (longer-lived caches) and **S13m** (export perf). S13a–j stay done. Do not start S15 while S13 is open. See §14.
...
| **S4a** | H&D + density/grade + cast 0.5 + BPC + OETF | First Python MAE / A/B (autos+Lab **off**) |
| **S4b** | Zone density/grade + CMY | One slider at a time vs Python |
| **S5** | Autos + `metering.py` remap | Auto Density while cropping |
...
| **S13** | Interactive + first-load performance | **Reopened** — S13a–j done (slider reprints + one decode + Accelerate + splash/thumbs + progressive first paint + GPU present + X-Trans PPG + queue/prefetch). **S13k–m** open (disk cache / optional later GPU / **export perf**). Not a look gate except S13i X-Trans PPG (keep S8/S14 MAE). |
...
S13a–j shipped (slider reprints + one linear decode per file + Accelerate convert/resize + embedded-preview splash + cheap strip thumbs + progressive draft / refine first paint + GPU present without float `getBytes` + X-Trans preview PPG + queue/prefetch). First open still has disk-cache work (S13k). In-process preview presents an Adobe RGB `CIImage` from an `rgba16Float` IOSurface. **Export / CLI stay CPU encoded readback** and, today, usually **full-res decode + full print pipeline** even for `target_px` / Quick Export (downsample is last). **S13m** closes that gap.

Sequence: **S13d → S13e → S13f → S13g** in parallel with **S13i → S13h → S13j → S13k → S13m**. **S13l** only if Instruments still shows those stages after S13d–h.
...
| **S13d** | One linear decode per file | **Done** — detect + open + render share one ImageIO/LibRaw pass (`LinearBufferCache` LRU; `DecodeReuseTests`; `PipelineStats.decode == 1`). Open armed-crop reuses the oversampled sample (not a separate 1800 px ImageIO pass). |
| **S13e** | Accelerate convert and resize | **Done** — `AccelerateConvert` (vImage extract / sRGB→linear / RGB↔RGBA; vDSP INTER_AREA; LibRaw `vDSP_vfltu16`). `AccelerateConvertTests`; linear / S8 / S14 MAE unchanged. |
| **S13f** | Splash and cheap thumbs | **Done** — `open(includeSplash: true)` returns LibRaw embedded JPEG (BITMAP falls back to the TIFF preview page). Strip `fast_preview` / `stripThumbnail` uses ImageIO thumbnail or RAW embedded JPEG plus a cheap invert — not H&D+Lab. `SplashThumbTests`. |
| **S13g** | Progressive / draft first paint | **Done** — Swift `selectFrame` paints a 512 processed draft (no analysis oversample, skip Lab sharpen / optical dust), then refines at 1600/2400. `NEGSWIFT_PERF_LOG` `native_first_paint` / `native_full_preview`. Settled look matches S8 (`ProgressivePaintTests`). |
| **S13h** | GPU present, no float readback | **Done** — in-process present is Adobe RGB `CIImage` / `CGImage` from an `rgba16Float` IOSurface (`LiteKernels.metallib`, reused ping/pong). First present does not `getBytes` the float working set; slider upload/download stays 0 (`GPUPresentTests`). CPU-vs-Metal MAE unchanged. Export / CLI stay encoded readback. |
| **S13i** | RAW decode | **Done** — X-Trans preview is PPG (1-pass Markesteijn), not AHD. One `libraw` handle per file shares `open`/`unpack` (`RawSessionCache`). Keep LibRaw OpenMP on the selected frame (do not pin `OMP_NUM_THREADS=1`). `RawDecodeTests`; S14 MAE oracles stay closed. |
...
S14 RAW look gates stay closed; S13 consumes them as oracles. `make compare-s13` is reprint + geometry + decode reuse + Accelerate convert + splash/thumbs + progressive paint + GPU present + RAW PPG (`RawDecodeTests`) + queue/prefetch (`QueuePrefetchTests`). Export perf is tracked under **S13m**, not S9.
