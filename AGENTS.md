...
| Native engine | `Packages/NegSwiftEngine` | Approved lite-path port (S0–S14); Python is oracle |
...
**Current status (2026-09-07):** M0–M15 feature complete. Native engine **S14** is in (LibRaw sensor-native camera RAW; ImageIO TIFF/JPEG unchanged). Preview/thumb RAW uses LibRaw `half_size` (Bayer LINEAR) or full-size PPG (X-Trans). Python remains the default. **S13** is reopened for first-load performance (S13a–k shipped; next **S13m** export perf). **S9** export look is done (`original` + `target_px`); Swift export still full-res pipeline unless S13m. **M12** manual benches remain — [docs/PERFORMANCE.md](docs/PERFORMANCE.md). Camera RAW on the Swift backend needs Homebrew `libraw` (`brew install libraw`). `NEGSWIFT_LIBRAW=0 swift test` still builds the stub. iOS is out of scope (GPL vs App Store).
...
- **Exception:** `Packages/NegSwiftEngine` may reimplement the NegSwift-lite path against Python goldens. Gate each vertical with the S0–S14 cards (pinned config, MAE, human A/B).
...
If the **Python** backend preview differs from NegPy desktop at the same config → **engine wiring bug**, not Swift UI bug. Fix the engine glue or config merge first.

If the **Swift** backend (`Packages/NegSwiftEngine`) differs from Python at the same pinned config → **port bug**. Do not “fix” it in the UI. Compare only at the vertical’s pinned keys (autos/Lab off until S5/S8).
...
NegSwift/
├── AGENTS.md              # this file
├── PLAN.md                # roadmap + milestones (M0–M15 + S0–S14 pointer)
├── NOTICE                 # NegPy upstream attribution
├── Vendor/NegPy/          # submodule (M9b+)
├── Packages/
│   └── NegSwiftEngine/    # native CPU engine (S0+; Python remains oracle)
...
1. **Implementing pipeline math in `App/` or `Engine/`** — import NegPy instead. The only approved Swift math is `Packages/NegSwiftEngine` (S0–S14).
...
| Doc | Purpose |
|-----|---------|
| [PLAN.md](PLAN.md) | Full roadmap, milestones M0–M15 + native engine S0–S14 |
...
| [docs/MANUAL_TEST_CHECKLIST.md](docs/MANUAL_TEST_CHECKLIST.md) | Manual smoke per milestone (M0–M15 and S0–S14) |
