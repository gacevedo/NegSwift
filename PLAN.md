...
| **S0–S13** Native Swift engine | **S7 done** | `.negpy` full-dict + Swift `serve --stdio`. Next: S8 Lab defaults. See §14 |

**Resume here:** Native engine **S8** (Lab saturation + sharpen 0.25 + skin 0.5). S7 sidecar + stdio is in. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md) for leftover M12 manual benches.
...
5. **Native engine S8:** Lab defaults (saturation + sharpen 0.25 + skin 0.5). S7 sidecar + stdio is in — see §14.
