...
| **S0–S15** Native Swift engine | **S13 reopened** | S13a–f done (slider reprints + one decode + Accelerate + splash/thumbs). Next: **S13g** progressive first paint. S14 RAW look is in. S15 later. See §14 |

**Resume here:** Native engine **S13g** (progressive / draft first paint). S13a–f shipped (slider reprints, one linear decode per file, vImage/vDSP convert/resize, embedded-preview splash + cheap strip thumbs). S14 camera RAW is in (oracle for S13i). S15 iOS stays later while S13 is open. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md).
...
5. **Native engine S13:** First-load performance — start at **S13g** (progressive first paint). S13a–f stay done. Do not start S15 while S13 is open. See §14.
...
| **S13** | Interactive + first-load performance | **Reopened** — S13a–f done (slider reprints + one decode + Accelerate + splash/thumbs). S13g–l open (first open). Not a look gate except S13i X-Trans PPG (keep S8/S14 MAE). |
...
S13a–f shipped (slider reprints + one linear decode per file + Accelerate convert/resize + embedded-preview splash + cheap strip thumbs). First open on the Swift backend is still slower than Python where X-Trans is full-size AHD, Metal `rgba32Float` upload/download + ColorSync `CGImage`, and there is no progressive first paint. Python keeps the GPU path on GPU.
...
S14 RAW look gates stay closed; S13 consumes them as oracles. `make compare-s13` is reprint + geometry + decode reuse + Accelerate convert + splash/thumbs.
