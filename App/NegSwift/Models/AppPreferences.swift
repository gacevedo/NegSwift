...
enum AppPreferencesStorage {
    ...
    /// Swift builds no longer offer a NegPy-desktop shared folder; preserve the path as custom.
    static func migrateAwayFromNegPyDesktopFolderIfNeeded() {
        #if !NEGSWIFT_ENGINE_PYTHON
        guard userDataLocation() == .negPyDesktop else { return }
        setCustomUserDataPath(negPyDesktopUserDirectory().path)
        setUserDataLocation(.custom)
        #endif
    }
...
final class AppPreferences {
    ...
    init() {
        AppPreferencesStorage.migrateAwayFromNegPyDesktopFolderIfNeeded()
