...
Run from the **NegSwift repo root**. Prefer an upstream **tag** (e.g. `0.58.0`); pin a specific commit only when validating main before a tag lands.

```bash
cd Vendor/NegPy && git fetch --tags && git checkout 0.58.0
cd ../..
...
**Tag vs commit:** `git checkout 0.58.0` checks out the tagged release. To pin main at a specific SHA: `git checkout <full-or-short-sha>`.
