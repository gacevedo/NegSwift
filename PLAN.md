...
| **S0–S15** Native Swift engine | **S13 reopened** | S13a–h done (slider reprints + one decode + Accelerate + splash/thumbs + progressive first paint + GPU present). Next: **S13i** X-Trans PPG. S14 RAW look is in. S15 later. See §14 |

**Resume here:** Native engine **S13i** (X-Trans preview PPG). S13a–h shipped (slider reprints, one linear decode per file, vImage/vDSP convert/resize, embedded-preview splash + cheap strip thumbs, progressive draft / refine first paint, GPU present without float readback). S14 camera RAW is in (oracle for S13i). S15 iOS stays later while S13 is open. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md).
...
5. **Native engine S13:** First-load performance — start at **S13i** (X-Trans preview PPG). S13a–h stay done. Do not start S15 while S13 is open. See §14.
...
| **S13** | Interactive + first-load performance | **Reopened** — S13a–h done (slider reprints + one decode + Accelerate + splash/thumbs + progressive first paint + GPU present). S13i–l open (first open). Not a look gate except S13i X-Trans PPG (keep S8/S14 MAE). |
...
S13a–h shipped (slider reprints + one linear decode per file + Accelerate convert/resize + embedded-preview splash + cheap strip thumbs + progressive draft / refine first paint + GPU present without float `getBytes`). First open on the Swift backend is still slower than Python where X-Trans is full-size AHD. In-process preview presents an Adobe RGB `CIImage` from an `rgba16Float` IOSurface. Export / CLI stay CPU readback.
...
S14 RAW look gates stay closed; S13 consumes them as oracles. `make compare-s13` is reprint + geometry + decode reuse + Accelerate convert + splash/thumbs + progressive paint + GPU present.
